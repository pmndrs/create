...
async function promptForOptions(name: string): Promise<GenerateOptions & { skipSetup: boolean }> {
  let cancelled = false
  if (name == null) {
    name = (
      await prompts(
  ...
  return {
    name,
    drei: answers.integrations?.includes('drei') ? true : undefined,
    handle: answers.integrations?.includes('handle') ? true : undefined,
    leva: answers.integrations?.includes('leva') ? true : undefined,
    postprocessing: answers.integrations?.includes('postprocessing') ? true : undefined,
    rapier: answers.integrations?.includes('rapier') ? true : undefined,
    xr: answers.integrations?.includes('xr') ? true : undefined,
    uikit: answers.integrations?.includes('uikit') ? true : undefined,
    offscreen: answers.integrations?.includes('offscreen') ? true : undefined,
    zustand: answers.integrations?.includes('zustand') ? true : undefined,
    koota: answers.integrations?.includes('koota') ? true : undefined,
    triplex: answers.integrations?.includes('triplex') ? true : undefined,
    viverse: answers.integrations?.includes('viverse') ? true : undefined,
...
async function main() {
  const program = new Command()
    ...
    .action(async (name: string = `react-three-${generateRandomName()}`, options: CliOptions) => {
      let generateOptions: GenerateOptions & { skipSetup?: boolean }
      ...
      if (options.url) {
        ...
        generateOptions = {
          name,
          drei: options.drei ? true : undefined,
          handle: options.handle ? true : undefined,
          leva: options.leva ? true : undefined,
          postprocessing: options.postprocessing ? true : undefined,
          rapier: options.rapier ? true : undefined,
          xr: options.xr ? true : undefined,
          uikit: options.uikit ? true : undefined,
          offscreen: options.offscreen ? true : undefined,
          zustand: options.zustand ? true : undefined,
          koota: options.koota ? true : undefined,
          viverse: options.viverse ? true : undefined,
          triplex: options.triplex ? true : undefined,
      ...
      try {
        const files = await generate(generateOptions)

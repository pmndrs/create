...
describe('@pmndrs/create', () => {
  ...
  describe('Integration tests', () => {
    ...
    it('should handle recipe requirements resolution with value modification', () => {
      ...
      expect(result).toEqual({
        'package.json': '{"name": "base-project", "dependencies": { "react": "^18.0.0", "react-router": "^6.0.0", "styled-components": "^5.0.0" }}',
  ...
  describe('Error handling', () => {
    it('should throw error for missing variables', () => {
      const recipe: Recipe = {
        name: 'missing-var-recipe',
        edits: {
          'index.js': 'console.log({{ @missingVariable }})',
        },
      }

      expect(() => buildProject([recipe])).toThrow('Variable "@missingVariable" not found or not yet resolved')
    })
    ...
    it('should throw error for circular variable referencing', () => {
      ...
      expect(() => buildProject([recipe])).toThrow('Circular dependency or missing variables detected for: @varA, @varB, @varC')
    })
    ...
    it('should throw error for self-referencing variable', () => {
      ...
      expect(() => buildProject([recipe])).toThrow('Circular dependency or missing variables detected for: @selfRef')
    })
    ...
    it('should throw error when pushing to undefined variable', () => {
      ...
      expect(() => buildProject([recipe])).toThrow('Cannot push to non-array value at path "@undefinedVar"')
    })
    ...
    it('should throw error when pushing to non-array variable', () => {
      ...
      expect(() => buildProject(recipes)).toThrow('Cannot push to non-array value at path "@stringVar"')
    })
    ...
    it('should throw error when pushing to object variable', () => {
      ...
      expect(() => buildProject(recipes)).toThrow('Cannot push to non-array value at path "@objectVar"')
    })
    ...
    it('should throw error when pushAll to non-array variable', () => {
      ...
      expect(() => buildProject(recipes)).toThrow('Cannot pushAll to non-array value at path "@numberVar"')
    })

    it('should handle writing to subpaths of non-object variables gracefully', () => {
      const recipe: Recipe = {
      ...
      // This should not throw but the nested path operation should be ignored
      const result = buildProject([recipe])
      expect(result).toEqual({})
    })

    it('should handle writing to subpaths of null variables', () => {
      const recipe: Recipe = {
        name: 'subpath-null-recipe',
        edits: {
          '@nullVar': { set: null },
          '@nullVar/nested': 'value',
        },
      }
    ...
    it('should handle complex circular dependency chains', () => {
      ...
      expect(() => buildProject([recipe])).toThrow('Circular dependency or missing variables detected')
    })
    ...
    it('should handle requirement resolution errors', () => {
      ...
      expect(() => buildProject([recipe])).toThrow('Recipe "missing-recipe" requires a resolveRequirements function to resolve dependencies')
    })
    ...
    it('should handle variable references in nested object structures', () => {
      ...
      expect(() => buildProject([recipe])).toThrow('Circular dependency or missing variables detected for: @config')
    })
    ...
    it('should handle multiple variable references in same string', () => {
      ...
      expect(() => buildProject([recipe])).toThrow('Variable "@missing" not found or not yet resolved')
    })
  })